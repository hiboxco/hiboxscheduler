    ...
    async resolve({ ctx, input }) {
      ...
      let apps = getApps(credentials).map(
        ({ credentials: _, credential: _1 /* don't leak to frontend */, ...app }) => ({
          ...
          credentialKeys: credentials.filter((c) => c.type === app.type).map((c) => c.key),
        })
      );
      if (variant) {
...
if (credential.type=="zoom_video"){
  const credkey = credential.key as unknown as CredKey;
  const accesstoken =  credkey?.access_token;
  //const accesstoken = credential.key?.access_token as Prisma.JSONValue;
