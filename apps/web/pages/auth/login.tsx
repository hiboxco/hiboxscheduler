...
export default function Login({
  ...
  const errorMessages: { [key: string]: string } = {
    ...
    //[ErrorCode.UserUnverified]: t("user_not_verified") + form.getValues("email"),
    [ErrorCode.UserUnverified]: t("user_not_verified"),
  ...
  useEffect(() => {
    if (!message && !messageDescription) {
      const msg =
        sessionStorage.getItem("hbus") == "yes" ? t("account_creation_heading") : t("sign_in_account");
      setMessage(msg);
      const msgdescription = sessionStorage.getItem("hbus") == "yes" ? t("account_creation_description") : "";
      setMessageDescription(msgdescription);
    }
    sessionStorage.removeItem("hbus");
  });
  ...
  const resendVerificationEmail = async (email: string) => {
    try {
      ...
      console.log(json);
      if (!res.ok) {
        setErrorMessage(`${t("something_went_wrong")} ${t("please_try_again_and_contact_us")}`);
      } else {
        setEmailVerificationMessage("Verification email has been sent! Please check your inbox");
      ...
      setErrorMessage(t("unexpected_error_try_again"));
    } finally {
      //setLoading(false);
    }
  };
  ...
  return (
    <>
      <AuthContainer
        ...
        description={t("login")}
        showLogo
        heading={twoFactorRequired ? t("2fa_code") : message}
        ...
        <Form
          ...
          handleSubmit={async (values) => {
            ...
            else if (res.error === ErrorCode.UserUnverified) setEmailVerificationPending(true);
          ...
          {emailVerificationPending && (
            <>
              <div className="gap-2">
                <span className="mb-2 block">{t("user_not_verified")}</span>

                <Button
                  className="block w-full justify-center border border-purple-200"
                  onClick={async () => {
          ...
          {emailVerificationMessage && <p className="mt-1 text-green-700">{emailVerificationMessage}</p>}
