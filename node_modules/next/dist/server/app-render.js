        ...
        function process() {
            forwardReader.read().then(({ done , value  })=>{
                ...
                if (done) {
                    ...
                    const css = responsePartial.split("\n").map((partialLine)=>{
                        const line = remainingFlightResponse + partialLine;
                        remainingFlightResponse = "";
                        try {
                            const match = line.match(/^M\d+:(.+)/);
                            if (match) {
                                return JSON.parse(match[1]).chunks.filter((chunkId)=>chunkId.endsWith(".css")).map((file)=>`<link rel="stylesheet" href="/_next/${file}">`).join("");
                            }
                            return "";
                        } catch (err) {
                            // The JSON is partial
                    ...
                    writer.write((0, _nodeWebStreamsHelper).encodeText(css + `<script>(self.__next_s=self.__next_s||[]).push(${(0, _htmlescape).htmlEscapeJsonString(JSON.stringify([
                        1,
                        id,
                        responsePartial
                    ]))})</script>`));
...
async function renderToHTML(req, res, pathname, query, renderOpts, isPagesDir) {
    ...
    segment)=>{
        ...
        const segmentParam = segment.startsWith("[") && segment.endsWith("]") ? segment.slice(1, -1) : null;
