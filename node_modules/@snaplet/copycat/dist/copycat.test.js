...
test("determinism", () => {
  ...
  const check = (name) => {
    const value1 = import_testutils.TRANSFORMATIONS[name](input);
    let i = -1;
    while (++i < import_testutils.NUM_CHECKS) {
      expect(import_testutils.TRANSFORMATIONS[name](input)).toEqual(value1);
    }
  };
...
test("generated values", () => {
  const count = 10;
  const results = {};
  const addResults = (name) => {
    const valueResults = [];
    let i = -1;
    while (++i < count) {
      valueResults.push(import_testutils.TRANSFORMATIONS[name](i));
    }
    results[name] = valueResults;
  };
  Object.keys(import_testutils.TRANSFORMATIONS).forEach(addResults);
