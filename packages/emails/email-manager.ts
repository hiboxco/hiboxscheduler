...
import VerificationEmail, { EmailVerification } from "./templates/verification-email";
...
export const sendVerificationEmail = async (emailVerificationTmp: EmailVerification) => {
  await new Promise((resolve, reject) => {
    try {
      const verificationEmail = new VerificationEmail(emailVerificationTmp);
      resolve(verificationEmail.sendEmail());
    } catch (e) {
      reject(console.error("OrganizerPaymentRefundFailedEmail.sendEmail failed", e));
    }
  });
